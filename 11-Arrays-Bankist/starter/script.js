...
const currentBalance = accInfo =>
  accInfo.movements.reduce((sum, mov) => sum + mov, 0);
...
const updateUI = function (acc) {
  ...
  curBalance = currentBalance(acc);
  labelBalance.textContent = `${curBalance}€`;
  //total deposit
...
const close = function(e){
  e.preventDefault();
  if(
    inputCloseUsername.value===currentAcc.userName && Number(inputClosePin.value) === currentAcc.pin
  ){
    const deleteAcc = accounts.findIndex(mov=> mov.userName === currentAcc.userName);
